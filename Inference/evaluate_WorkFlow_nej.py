...
def getFinalSystemPrompt(system_prompt, results, llm_scores):
    """Construct a system prompt for layers 2+ that includes the previous responses to synthesize."""
    return (
        system_prompt
        + "\n"
        + "\n".join([f"{i+1}. {str(element)} This respondent's historical accuracy rate for this category of question is {str(score)}" for i, (element, score) in enumerate(zip(results, llm_scores))])
    )

import gc

async def run_llm( in_model, patient_response=None, prev_response=None, judge_promt = None, error_promt = None, role_prompt=None, llm_scores=None):
    # print("🪪 model_name:", in_model.model_type, file=file_out)
    print("🪪 model_name:", in_model.model_type)
    for sleep_time in [1, 2, 4]:
        try:
            if judge_promt:
                assistant_sys_msg =getFinalSystemPrompt(judge_promt, prev_response, llm_scores)
                # print("🪪 assistant_sys_msg:", assistant_sys_msg)
                ...
                assistant_sys_msg =getFinalSystemPrompt(MOA_sys_msg, prev_response, llm_scores)
                # print("🪪 assistant_sys_msg:", assistant_sys_msg)
            ...
            if "CUDA error: out of memory" in error_str or "out of memory" in error_str:
                # 对于内存错误，等待时间更长
                wait_time = 2 * (2 ** sleep_time)  # 指数退避: 2, 4, 8, 16...秒
                # print(f"CUDA内存不足，等待{wait_time}秒后重试...", file=file_out)
                print(f"CUDA内存不足，等待{wait_time}秒后重试...")
                await asyncio.sleep(wait_time)
            else:
                # 其他错误使用较短的等待时间
                wait_time = 2 * (sleep_time + 1)  # 线性递增: 2, 4, 6, 8...秒
                await asyncio.sleep(wait_time)
...
# MOA_sys_msg2 = """You have been provided with the summary of a set of responses from various open-source models to the latest user query. Your task is to refer the summary and generate a single, high-quality response.  It is crucial to critically evaluate the information provided in the summary, recognizing that some of it may be biased or incorrect. Your response should not simply replicate the given answers but should offer a refined, accurate, and comprehensive reply to the instruction. Ensure your response is well-structured, coherent, and adheres to the highest standards of accuracy and reliability. Don't output content irrelevant to the question. The final answer has only one choice and needs to be provided in JSON format.
...
# judg_question = """Please score based on the quality of the response."""
...
async def batch_inference(llm, inf_prompt, question, dept, diff, llm_scores):
    layers = 3
    ...
    Antagonist = await asyncio.gather(*[run_llm(in_model = Antagonist_model, prev_response = results, judge_promt=agst_prompt, llm_scores=llm_scores)])
    Antagonist = remove_think_tags(Antagonist)
    print('Antagonist1::::::::::::::::::::::::::',Antagonist)
    # results= results + Antagonist
    
    for _ in range(1, layers-1):
        print("🪜 Layer:", _)
        print("🪜 Layer:", _)

        results = await asyncio.gather(*[run_llm(in_model = model, patient_response = question, prev_response = results, error_promt=Antagonist, role_prompt = dept_diff_promt, llm_scores=llm_scores) for model in reference_models])
        results = remove_think_tags(results)
    ...
    # aggregator_model
    print("🪜 aggregator", _)
    results = await asyncio.gather(*[run_llm(in_model = aggregator_model, patient_response = question, prev_response = results, error_promt=Antagonist, role_prompt = dept_diff_promt, llm_scores=llm_scores)])
    results = remove_think_tags(results)
...
with open("./NEJclass.json", 'r', encoding='utf-8') as file:
    class_data = json.load(file)


async def main():
    with open("WorkFlow32b_nejm0602_test.txt", "w") as f:
        for i, item in enumerate(data):
            ...
            results = await asyncio.gather(batch_inference(llm = top_models, inf_prompt = Answer_Prompt, question = Question_Prompt, dept = med_depart, diff = complexity, llm_scores = scores))
            f.write(str(results) + "\n")
